...
class ShipperSignUp extends StatefulWidget {
  final VoidCallback onTapSignUp;
  const ShipperSignUp({super.key, required this.onTapSignUp});

  @override
  State<ShipperSignUp> createState() => _ShipperSignUpState();
}

class _ShipperSignUpState extends State<ShipperSignUp> {
  ...
  // Optional extra file (for example, a shipping permit or additional document)
  ...
  /// When [isProfileImage] is true, the file is used as the profile image;
  /// otherwise, it is used as the extra file.
  ...
  Future<void> _uploadProfileImageToFirebase() async {
    if (_selectedProfileImageBytes == null ||
        _selectedProfileImageName == null) return;
    try {
  ...
  /// Uploads the extra file (if any) to Firebase Storage.
  ...
  /// Creates a new user with Firebase Auth and saves additional shipper data to Firestore.
  ...
  /// Helper method to build input fields with a label and validation.
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildInputField('Email', _emailController, 'Enter your email', validator: (value) {
            ...
            SizedBox(height: 24.h),
            // Optional File Picker (for additional certifications)
            Text(
