...
class DriverSignUp extends StatefulWidget {
  ...
  const DriverSignUp({super.key, required this.onTapSignUp});
...
class _DriverSignUpState extends State<DriverSignUp> {
  ...
  final TextEditingController _nameController = TextEditingController(); // Full Name
  ...
  final TextEditingController _locationPasswordController = TextEditingController();
  final TextEditingController _experienceController = TextEditingController();
  final TextEditingController _specializationController = TextEditingController();
  final TextEditingController _employerController = TextEditingController();
  final TextEditingController _contact_detailsController = TextEditingController();
  final TextEditingController _truck_modelController = TextEditingController();
  final TextEditingController _trailer_typesController = TextEditingController();
  ...
  // Profile Image data (if you need a profile photo for drivers)
  ...
  /// When isProfileImage is true, the picked file will be used as a profile image;
  /// otherwise (false) it is used for the driving license.
  ...
  /// (if your driver signup allows an optional profile photo)
  ...
  /// Creates a new user with Firebase Authentication and uploads driver data to Firestore
  Future<void> _uploadUserData() async {
    ...
    try {
      ...
      await FirebaseFirestore.instance.collection('users').doc(uid).set({
        ...
        'license_class': _licenseClassController.text.trim(),
        'endorsements': _endorsementsController.text.trim(),
        'driving_license_url': _uploadedFileUrl,
        'account_type': 'Driver',
        'isApprove': false,
        "location": _locationPasswordController.text.trim(),
        // Optionally, include profile_image_url if available.
        'profile_image_url': _uploadedProfileImageUrl,
        'experience': _experienceController.text.trim(),
        'specialization': _specializationController.text.trim(),
        'employer': _emailController.text.trim(),
        'contact_details': _contact_detailsController.text.trim(),
        'truck_model': _truck_modelController.text.trim(),
        'trailer_types': _trailer_typesController.text.trim(),
  ...
  /// Handles the complete signup flow: validates the form, uploads files, and saves user data.
  Future<void> _handleSignup() async {
    if (_formKey.currentState!.validate()) {
      // Optional: Upload profile image if provided
      await _uploadProfileImageToFirebase();
      // Upload driving license file if provided
      await _uploadDrivingLicenseToFirebase();
      // Create new user and upload data to Firestore
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Center(
              child: Text(
                'Create your driver account and get verified to start hauling loads.',
            ...
            // Optional profile image picker (tap to select photo)
            Center(
              child: GestureDetector(
                ...
                child: CircleAvatar(
                  ...
                  backgroundImage: _selectedProfileImageBytes != null ? MemoryImage(_selectedProfileImageBytes!) : null,
            ...
            SizedBox(height: 24.h),
            _buildInputField('License class', _licenseClassController, 'Enter your License class'),
            SizedBox(height: 24.h),
            _buildInputField('Endorsements', _endorsementsController, 'Enter endorsements'),
            _buildInputField('Location', _locationPasswordController, 'Enter location'),
            _buildInputField('Experience', _experienceController, 'Enter experience'),
            _buildInputField('Specialization', _specializationController, 'Enter specialization'),
            _buildInputField('Employer', _employerController, 'Enter employer'),
            _buildInputField('Contact_details', _contact_detailsController, 'Enter contact details'),
            _buildInputField('Truck_model', _truck_modelController, 'Enter truck model'),
            _buildInputField('Trailer_types', _trailer_typesController, 'Enter trailer ypes'),
            _buildInputField('Maintenance', _maintenanceController, 'Enter maintenance'),
            _buildInputField('Certification', _certificationController, 'Enter certification'),
            _buildInputField('School', _schoolController, 'Enter school'),
            _buildInputField('Award', _awardController, 'Enter award'),
            SizedBox(height: 24.h),
            _buildInputField(
              'Password',
              _passwordController,
            ...
            SizedBox(height: 24.h),
            Text(
              'Why Verify?\nOnce your license is approved, your profile will display a badge showing you’re ready to work!',
              style: TextStyle(
                color: const Color(0xFFC4C4C4),
                fontSize: 16.sp,
                fontWeight: FontWeight.w400,
  ...
  /// Helper method to build input fields (duplicated for convenience)
