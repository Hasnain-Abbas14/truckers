...
class _CreatePostState extends State<CreatePost> {
  ...
  Future<String?> _uploadFileToFirebase() async {
    ...
    try {
      ...
      // Start upload
      if (kIsWeb) {
        _uploadTask = storageRef.putBlob(_pickedFile as html.File);
      } else {
        _uploadTask = storageRef.putFile(_pickedFile as File);
      }
      ...
      _uploadTask!.snapshotEvents.listen((event) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Uploading: ${(_progress * 100).toStringAsFixed(1)}%'),
  ...
  Future<void> _saveToFirebase() async {
    ...
    try {
      ...
      if (_pickedFile != null) {
        fileUrl = await _uploadFileToFirebase();
        if (fileUrl == null) {
      ...
      // Call the callback to navigate to LoadSecurement
                              ...
                              child: Row(
                                ...
                                children: [
                                  ...
                                  Column(
                                    ...
                                    children: [
                                      Text(
                                        _pickedFile == null ? 'Upload File (Optional)' : 'File Selected',
                                        style: AppTextStyles.mulishStyle.copyWith(
                                          color: const Color(0xFF262626),
                                          fontSize: 26.sp,
                                          fontWeight: FontWeight.w700,
